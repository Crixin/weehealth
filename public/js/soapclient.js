...
SOAPClient._node2object = function(node, wsdlTypes)
{
	...
	if(!isarray)
	{
		...
		for(var i = 0; i < node.childNodes.length; i++)
		{
			...
			var arraysObjetos = ['listaIndicesRegistro', 'listaIndiceDocumento', 'listaIndice', 'listaRegistro', 'listaDocumento']; 
			
			if(arraysObjetos.indexOf(node.childNodes[i].nodeName) !== -1){
				if(!obj[node.childNodes[i].nodeName]) obj[node.childNodes[i].nodeName] = [];
				obj[node.childNodes[i].nodeName].push(p);
			} else {	
				obj[node.childNodes[i].nodeName] = p;
			}
			
			/* FIM ALTERAÇÕES PRINTMAX */
		}
