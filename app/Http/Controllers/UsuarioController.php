...
use App\Repositories\{UserRepository, PerfilRepository};
...
class UsuarioController extends Controller
{
    ...
    public function updateUser(Request $request)
    {
        ...
        if ($request->get('username') != $usuario->username) $arrRegras['username'] = 'required|string|max:20|unique:users';
        if ($request->get('email') != $usuario->email)       $arrRegras['email'] = 'required|string|email|max:255|unique:users';
