...
use Helper;
use Session;
use App\{User, Grupo, Cidade, Empresa, Processo, EmpresaUser, EmpresaGrupo, EmpresaProcesso};
...
class EmpresaController extends Controller
{
    ...
    */
    ...
    public function updateEnterprise(Request $request) {
        ...
        if($empresa->nome != $request->get('nome')) $arrRegras['nome'] = 'required|string|max:100|unique:empresa';
    ...
    public function linkedUsers($_id) {
        ...
        $usuariosJaVinculados = EmpresaUser::join('users', 'users.id', '=', 'user_id')
                                    ...
                                    ->select('empresa_user.id', 'permissao_download', 'permissao_visualizar', 'permissao_impressao', 'permissao_aprovar_doc', 'permissao_excluir_doc', 'permissao_upload_doc', 'permissao_receber_email', 'name', 'email', 'user_id', 'empresa_id', 'permissao_editar')->get();
        $idUsuariosJaVinculados = $usuariosJaVinculados->pluck('user_id');
        $usuariosRestantes = User::select('id', 'name')->whereNotIn('id', $idUsuariosJaVinculados)->orderBy('name')->get();
    ...
    public function linkedGroups($_id) {
        ...
        $gruposJaVinculados = EmpresaGrupo::join('grupo', 'grupo.id', '=', 'grupo_id')
                                    ->where('empresa_id', '=', $empresa->id)
                                    ->select('empresa_grupo.id', 'permissao_download', 'permissao_visualizar', 'permissao_impressao', 'permissao_aprovar_doc', 'permissao_excluir_doc', 'permissao_upload_doc', 'permissao_receber_email', 'nome', 'grupo_id', 'empresa_id', 'permissao_editar')->get();
